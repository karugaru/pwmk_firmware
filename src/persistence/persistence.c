...
#define DEBUG_PRINT(...) pwmk_debug_printf("PERSISTENCE", __VA_ARGS__)
#else
...
bool persistence_init(size_t image_size) {
  ...
  DEBUG_PRINT("init: image_size=%zu result=%d\n", image_size, result);
  return result;
}
...
bool persistence_restore(uint8_t *image, size_t image_size) {
  DEBUG_PRINT("restore: image_size=%zu\n", image_size);
  if (!_persistence_image_is_valid(image, image_size)) {
    DEBUG_PRINT("restore failed: invalid image\n");
    return false;
  }
  ...
  if (!persistence_flash_read(persistence_flash_layout.marker_offset, marker,
                              sizeof(marker))) {
    DEBUG_PRINT("restore failed: marker read\n");
    return false;
  }
  // マーカーが一致しない場合は復元を失敗させる
  if (memcmp(marker, PWMK_PERSISTENCE_MARKER, sizeof(marker)) != 0) {
    DEBUG_PRINT("restore failed: marker mismatch\n");
    return false;
  }
  ...
  if (!_persistence_read_progress(&progress_info)) {
    DEBUG_PRINT("restore failed: progress read\n");
    return false;
  }
  if (progress_info.state != PWMK_PROGRESS_COMPLETE ||
      progress_info.completed_save_count == 0u) {
    DEBUG_PRINT("restore failed: progress state=%d saves=%zu\n",
  ...
  if (!persistence_flash_read(persistence_flash_layout.built_data_offset, image,
                              image_size)) {
    DEBUG_PRINT("restore failed: built data read\n");
    return false;
  }
  ...
  if (!persistence_flash_get_view(persistence_flash_layout.log_data_offset,
                                  ...
                                  &read_log_address)) {
    DEBUG_PRINT("restore failed: log view\n");
    return false;
  }
  ...
  if (!persistence_format_log_replay(read_log_address,
    ...
    DEBUG_PRINT("restore failed: log replay\n");
    return false;
  }
  ...
  DEBUG_PRINT("restore complete: saves=%zu log_used=%zu\n",
...
bool persistence_commit(const uint8_t *image, size_t image_size) {
  DEBUG_PRINT("commit: image_size=%zu\n", image_size);
  // 永続化可能かどうかをチェックする
  if (!_persistence_image_is_valid(image, image_size)) {
    DEBUG_PRINT("commit failed: invalid image\n");
    return false;
  }
  if (!persistence_state.available) {
    DEBUG_PRINT("commit failed: persistence unavailable\n");
    return false;
  }
  ...
  if (!persistence_restore(saved_image, image_size)) {
    DEBUG_PRINT("commit failed: restore current image\n");
  ...
  DEBUG_PRINT("commit diff: offset=%zu size=%zu\n", offset, remaining);
  while (remaining > 0u) {
    ...
    if (!persistence_format_log_encode(record, sizeof(record), &record_size,
      ...
      DEBUG_PRINT("commit failed: log encode offset=%zu size=%zu\n", offset,
                  data_size);
      persistence_state.available = false;
      return false;
    }
  ...
  DEBUG_PRINT("commit complete: saves=%zu log_used=%zu\n",
              persistence_state.completed_save_count,
              persistence_state.log_used_size);
  return true;
}
...
bool persistence_rebuild(const uint8_t *image, size_t image_size) {
  DEBUG_PRINT("rebuild: image_size=%zu\n", image_size);
  persistence_state.available = false;
  if (!_persistence_image_is_valid(image, image_size)) {
    DEBUG_PRINT("rebuild failed: invalid image\n");
    return false;
  }

  // フラッシュ領域を消去する
  if (!persistence_flash_erase_all()) {
    DEBUG_PRINT("rebuild failed: flash erase\n");
    return false;
  }
  // マーカーを書き込む
  if (!persistence_flash_program(persistence_flash_layout.marker_offset,
    ...
    DEBUG_PRINT("rebuild failed: marker write\n");
    return false;
  }
  // シリアル番号を書き込む
  if (!persistence_flash_program(persistence_flash_layout.serial_offset,
    ...
    DEBUG_PRINT("rebuild failed: serial write\n");
    return false;
  }
  // 保存進捗を保存中にする
  if (!_persistence_update_progress(0u, false)) {
    DEBUG_PRINT("rebuild failed: progress start\n");
    return false;
  }
  // 構築済みデータを書き込む
  if (!persistence_flash_program(persistence_flash_layout.built_data_offset,
                                 image, image_size)) {
    DEBUG_PRINT("rebuild failed: built data write\n");
    return false;
  }
  // 保存進捗を保存完了にする
  if (!_persistence_update_progress(0u, true)) {
    DEBUG_PRINT("rebuild failed: progress complete\n");
    return false;
  }
  // 保存進捗を読み込んで、正しく初期化されたか確認する
  pwmk_progress_info_t progress_info;
  if (!_persistence_read_progress(&progress_info)) {
    DEBUG_PRINT("rebuild failed: progress verify read\n");
    return false;
  }
  if (progress_info.state != PWMK_PROGRESS_COMPLETE ||
      progress_info.completed_save_count != 1u) {
    DEBUG_PRINT("rebuild failed: progress verify state=%d "
                "saves=%zu\n",
                progress_info.state, progress_info.completed_save_count);
    return false;
  }
...
static bool _persistence_update_progress(size_t slot, bool mark_completed) {
  ...
  if (!persistence_format_progress_get_slot_update(slot, mark_completed,
    ...
    DEBUG_PRINT("progress update failed: slot=%zu completed=%d\n", slot,
                mark_completed);
    return false;
  }
  ...
  if (!persistence_flash_read(offset, &current_value, sizeof(current_value))) {
    DEBUG_PRINT("progress update failed: read offset=%zu\n", offset);
    return false;
  }
  ...
  if (!persistence_format_progress_update_slot(
    ...
    DEBUG_PRINT("progress update failed: invalid value slot=%zu "
                "current=0x%02X\n",
                slot, current_value);
    return false;
  }
  ...
  DEBUG_PRINT("progress update: slot=%zu completed=%d "
...
static bool _persistence_read_progress(pwmk_progress_info_t *info) {
  ...
  if (!persistence_flash_read(persistence_flash_layout.progress_offset,
    ...
    DEBUG_PRINT("progress read failed\n");
    return false;
  }
  ...
  if (result) {
    DEBUG_PRINT("progress: state=%d saves=%zu\n", info->state,
                info->completed_save_count);
  } else {
    DEBUG_PRINT("progress parse failed\n");
  }
  return result;
}
...
static bool _persistence_append_log(const uint8_t *record, size_t record_size) {
  if (record == NULL || !_persistence_can_append(record_size)) {
    DEBUG_PRINT("log append failed: record_size=%zu\n", record_size);
    return false;
  }
  ...
  if (!persistence_format_progress_get_next_slot(&current_progress, &slot)) {
    DEBUG_PRINT("log append failed: no progress slot\n");
    return false;
  }

  DEBUG_PRINT("log append: slot=%zu offset=%zu size=%zu\n", slot,
  ...
  if (!_persistence_update_progress(slot, false)) {
    DEBUG_PRINT("log append failed: progress start\n");
    return false;
  }
  // 書き込みログをフラッシュに書き込む
  if (!persistence_flash_program(persistence_flash_layout.log_data_offset +
                                     persistence_state.log_used_size,
                                 record, record_size)) {
    DEBUG_PRINT("log append failed: log write\n");
    return false;
  }
  // 保存進捗を保存完了にする
  if (!_persistence_update_progress(slot, true)) {
    DEBUG_PRINT("log append failed: progress complete\n");
    return false;
  }
  ...
  if (!_persistence_read_progress(&updated_progress)) {
    DEBUG_PRINT("log append failed: progress verify read\n");
    return false;
  }
  if (updated_progress.state != PWMK_PROGRESS_COMPLETE ||
      updated_progress.completed_save_count !=
          persistence_state.completed_save_count + 1u) {
    DEBUG_PRINT("log append failed: progress verify state=%d "
                "saves=%zu\n",
  ...
  DEBUG_PRINT("log append complete: saves=%zu log_used=%zu\n",
              persistence_state.completed_save_count,
              persistence_state.log_used_size);
  return true;
}
