...
USB接続とBLE接続は、CMakeオプションで個別に切り替えられます。

- USBとBLEの両方を有効にする: 既定値のままビルドします。
- USBのみ有効にする: CMake configure時に`-DPWMK_ENABLE_BLE=OFF`を指定します。
- BLEのみ有効にする: CMake configure時に`-DPWMK_ENABLE_USB=OFF`を指定します。

USB接続を無効にしたビルドでは、`stdio_usb`が自動で有効になります。
